...
class imobile_Core {
    public static function itemlink($item,$n) {
        ...
        if ($item->thumb_width <= $item->thumb_height) {
            ...
            $sqleft = 0;
            $sqtop = (int) ($thsize/2 - ($item->thumb_height * ($thsize / $item->thumb_width)) / 2);
        } else {
            $size = "height:".$thsize."px";
            $sqleft = (int) ($thsize/2 - ( ($item->thumb_width * ($thsize / $item->thumb_height)) / 2) );
            $sqtop = 0;
        }
        ...
        if ($item->is_album()) {
            ...
            if ($item->is_movie()) {
                ...
                // uncomment the following and comment the other one to enable full size images in slideshow mode
                // full size images in slideshow mode
                //$arguments[] = "'".$item->file_url()."'";
                // resized images in slideshow mode

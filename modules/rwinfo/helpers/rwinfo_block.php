  ...
  static function get($block_id, $theme) {
    ...
    case "rwinfo":
      if ($theme->item()) {
        ...
        if (!$theme->item->is_album() && $theme->item->captured && module::get_var("rwinfo", "show_captured")) {
          $info["captured"] = array(
            "label" => t("Date:"),
            "value" => gallery::date_time($theme->item->captured)
          );
        }
        else if (!$theme->item->is_album() && $theme->item->created && module::get_var("rwinfo", "show_captured")) {
          $info["captured"] = array(
            "label" => t("Date:"),
            "value" => gallery::date_time($theme->item->created)
          );
        }
        // End rWatcher Edit
